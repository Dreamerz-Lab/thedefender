...
public static class I360Render {
    ...
    public static byte[] Capture(ref Texture2D texture2D, int width = 1024, bool encodeAsJPEG = true, Camera renderCam = null, bool faceCameraDirection = true) {
        //Texture2D output = null;

        //if (texture2D != null)
        //    output = texture2D;

        Debug.Log("Called Capture");
        ...
        try {
            ...
            if (!texture2D)
                texture2D = new Texture2D(equirectangularTexture.width, equirectangularTexture.height, TextureFormat.RGB24, false);
            else if (texture2D.width != cubemapSize) {
                UnityEngine.Object.DestroyImmediate(texture2D);
                texture2D = new Texture2D(equirectangularTexture.width, equirectangularTexture.height, TextureFormat.RGB24, false);
            }

            texture2D.ReadPixels(new Rect(0, 0, equirectangularTexture.width, equirectangularTexture.height), 0, 0);
